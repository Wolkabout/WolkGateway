...
public:
    ...
    disconnect_options(const std::chrono::duration<Rep, Period>& to, const token_ptr& tok) : disconnect_options()
    {
        set_timeout(to);
        set_token(tok);
    }
     ...
     */
    std::chrono::milliseconds get_timeout() const { return std::chrono::milliseconds(opts_.timeout); }
    /**
