...
namespace wolkabout
{
FileDownloadService::FileDownloadService(uint_fast64_t maxFileSize, uint_fast64_t maxPacketSize, std::unique_ptr<FileHandler> fileHandler,
...
{
	addToCommandBuffer([=]{
		m_currentState->handleBinaryData(binaryData);
	});
}
...
void FileDownloadService::IdleState::download(const std::string& fileName, uint_fast64_t fileSize, const ByteArray& fileHash,
											  const std::string& downloadDirectory, std::function<void(const std::string& filePath)> onSuccessCallback,
											  std::function<void(WolkaboutFileDownloader::Error errorCode)> onFailCallback)
{
...
void FileDownloadService::DownloadState::download(const std::string&, uint_fast64_t, const ByteArray&,
