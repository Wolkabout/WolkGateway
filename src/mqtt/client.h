...
class client : private callback
{
     ...
     */
    template <typename T> std::shared_ptr<T> ptr(const T& val)
    {
    ...
    void message_arrived(const_message_ptr msg) override { userCallback_->message_arrived(msg); }
...
public:
     ...
     *  				  then no persistence is used.
     */
    client(const string& serverURI, const string& clientId, int maxBufferedMessages,
     ...
     *  			  of zero or less means the client will not quiesce.
     */
    template <class Rep, class Period> void disconnect(const std::chrono::duration<Rep, Period>& to)
    {
     ...
     */
    virtual void publish(const_message_ptr msg) { cli_.publish(msg)->wait_for(timeout_); }
    /**
     * Publishes a message to a topic on the server.
     ...
     */
    virtual void set_timeout(int timeoutMS) { timeout_ = std::chrono::milliseconds(timeoutMS); }
    /**
     * Set the maximum time to wait for an action to complete.
     * @param to The timeout as a std::chrono duration.
     */
    template <class Rep, class Period> void set_timeout(const std::chrono::duration<Rep, Period>& to)
    {
     ...
     * @param topicFilter
     */
    virtual void subscribe(const string& topicFilter) { cli_.subscribe(topicFilter, DFLT_QOS)->wait_for(timeout_); }
    /**
     * Subscribe to a topic, which may include wildcards.
     * @param topicFilter A single topic to subscribe
     * @param qos The QoS of the subscription
     */
    virtual void subscribe(const string& topicFilter, int qos) { cli_.subscribe(topicFilter, qos)->wait_for(timeout_); }
    /**
     ...
     */
    virtual void unsubscribe(const string& topicFilter) { cli_.unsubscribe(topicFilter)->wait_for(timeout_); }
    /**
     ...
     *  	   available.
     */
    bool try_consume_message(const_message_ptr* msg) { return cli_.try_consume_message(msg); }
    /**
