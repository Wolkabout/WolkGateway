...
public:
     ...
     */
    static ptr_t create(iasync_client& cli) { return std::make_shared<token>(cli); }
    /**
     * Constructs a token object.
     * @param cli The client that created the token.
     * @param userContext optional object used to pass context to the
     ...
     * @param topic The topic assiciated with the token
     */
    static ptr_t create(iasync_client& cli, const string& topic) { return std::make_shared<token>(cli, topic); }
    /**
     * Constructs a token object.
     * @param cli The client that created the token.
     * @param topic The topic assiciated with the token
     ...
     *  	   timeout occurred.
     */
    virtual bool wait_for(long timeout) { return wait_for(std::chrono::milliseconds(timeout)); }
    /**
     ...
     */
    template <class Rep, class Period> bool wait_for(const std::chrono::duration<Rep, Period>& relTime)
    {
        unique_lock g(lock_);
        if (!cond_.wait_for(g, std::chrono::milliseconds(relTime), [this] { return complete_; }))
            return false;
        check_rc();
        return true;
    }
     ...
     *  	   @em false on a timeout.
     */
    template <class Clock, class Duration> bool wait_until(const std::chrono::time_point<Clock, Duration>& absTime)
    {
        unique_lock g(lock_);
