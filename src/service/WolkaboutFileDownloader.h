...
public:
     ...
     * Should handle multiple parllel downloads
     ...
     * @param subChannel Sub channel on which to request file
    ...
    virtual void download(const std::string& fileName, std::uint_fast64_t fileSize, const ByteArray& fileHash,
                          ...
                          std::function<void(WolkaboutFileDownloader::ErrorCode errorCode)> onFailCallback) = 0;

    /**
     * @brief abort Aborts file download and removes any saved data
     * Should be implemented in a thread safe manner
     * @param subChannel Sub channel on which file is being downloaded
     */
    virtual void abort(const std::string& subChannel) = 0;
};
}    // namespace wolkabout
