...
public:
     ...
     */
    explicit exception(int code) : std::runtime_error("MQTT error [" + std::to_string(code) + "]"), code_(code) {}
    /**
     * Creates an MQTT exception.
     * @param code The error code from the C library.
    ...
    : std::runtime_error("MQTT error [" + std::to_string(code) + "]: " + msg), code_(code), msg_(msg)
    {
    }
    /**
...
public:
     ...
     * @param msg The text message for the error.
     */
    explicit persistence_exception(const string& msg) : exception(MQTTCLIENT_PERSISTENCE_ERROR, msg) {}
    /**
     * Creates an MQTT persistence exception.
