...
public:
     ...
     *  				  then no persistence is used.
     */
    async_client(const string& serverURI, const string& clientId, int maxBufferedMessages,
     ...
     * @throw exception for problems encountered while disconnecting
     */
    template <class Rep, class Period> token_ptr disconnect(const std::chrono::duration<Rep, Period>& timeout)
    {
        // TODO: check range
     ...
     * @throw exception for problems encountered while disconnecting
     */
    token_ptr disconnect(void* userContext, iaction_listener& cb) override { return disconnect(0L, userContext, cb); }
    /**
    ...
    {
        return publish(std::move(topic), std::move(payload), message::DFLT_QOS, message::DFLT_RETAINED);
    }
    /**
     * Publishes a message to a topic on the server
     * @param topic The topic to deliver the message to
     ...
     *  	   token will be passed to callback methods if set.
     */
    delivery_token_ptr publish(string_ref topic, const void* payload, size_t n, int qos, bool retained,
     ...
     *  	   The token will be passed to callback methods if set.
     */
    token_ptr subscribe(const_string_collection_ptr topicFilters, const qos_collection& qos, void* userContext,
     ...
     *  	   available.
     */
    bool try_consume_message(const_message_ptr* msg) { return que_->try_get(msg); }
    /**
